...
exports.updateUser = async (params) => {
  ...
  const { userId, fname, email, password } = params;
  ...
  return new Promise((resolve, reject) => {
    db.query(
      ...
      (err, result) => {
        ...
        if (result.length === 0) {
          reject({
            message: "Wrong credentials1111, please try again",
            statusCode: 400,
          });
        } else {
          if (email === result[0].email && fname === result[0].fname) {
            reject({
